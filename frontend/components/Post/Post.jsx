...
const Post = (props) => {
    ...
    const { _id, imageURL, title, tags, likes, comments, description, ownerName, ownerId } = props.postData
    // console.log(ownerId);
    return (
        <View className=' mb-1 flex flex-col'>
            ...
            <View className={`flex flex-row justify-between items-center w-[${widthOfScreen}px] h-14 px-2 `}>
                ...
                <View className='flex flex-row justify-center items-center'>
                    ...
                    >
                        {ownerId?.userImage && <Image source={{ uri: ownerId?.userImage }} className="h-full w-full rounded-full object-cover " />}

                    </LinearGradient>
                    <View className='ml-1 flex flex-col justify-start items-start'>
                        <Text className='text-white  text-sm font-Nunito_Medium tracking-tight lowercase'>{ownerName}</Text>
